...
def create():
    ...
    if request.method == 'POST':
        ...
        mutante = MutanteModel(nome=nome, codinome=codinome)
...
def update(id):
    mutante = MutanteModel.query.filter_by(id=id).first()
    if request.method == 'POST':
        if mutante:
            ...
            nome = request.form['nome']
            codinome = request.form['codinome']
            mutante = MutanteModel(id=id, nome=nome, codinome=codinome)
            db.session.add(mutante)
            db.session.commit()
            return redirect(f'/xmen/{id}')
        return f"Mutante with id = {id} Does nit exist"
 
    return render_template('editar.html', mutante = mutante)
